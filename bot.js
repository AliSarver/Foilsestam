...
client.on('message', ra3d => { // Leaked by [ @Fr3on Gamer#9338 ]
  ...
  client.on('message' , ReBeL => { // Leaked by [ @FOIL#4996 ]
var prefix = "-";
...
client.on("guildMemberAdd", member => { // Leaked by [ @FOIL#4996 ]
...
client.on('message', message => { // Leaked by [ @FOIL#4996 ]
...
client.on("message",function(message) { // Leaked by [ @FOIL#4996 ]
...
message.channel.send(`let args = message.content.split(" ").slice(${args}).join(" ")`);
    ...
    if(message.content.startsWith(prefix + "server")) { // Leaked by [ @FOIL#4996 ]
...
client.on('guildCreate', guild => { // Leaked by [ @FOIL#4996 ]
var message = "zg";
         const embed = new Discord.RichEmbed()
     .setColor("GOLD")
     .setTitle('FOIK Bot Link Click Here .!')
     .setURL('https://discordapp.com/api/oauth2/authorize?client_id=498254846819041287&permissions=8&scope=bot')
  .setDescription(`**
...
client.on('guildDelete', guild => { // Leaked by [ @FOIL#4996 ]
var message = "zg";
         const embed = new Discord.RichEmbed()
     .setColor("GOLD")
     .setTitle('FOIL Bot Link Click Here .!')
     .setURL('https://discord.gg/AQYpbjY')
  .setDescription(`**
  Server Kicked FOIL Bot :cry:
اسم السيرفر: ${guild.name}
صاحب السيرفر: ${guild.owner}**`);
client.channels.get("465340887279468564").sendEmbed(embed)
});
...
client.on('message', async message => { // Leaked by [ @FOIL#4996 ]
...
coll.first().delete()
  ...
  e.edit("تم انشاء الغرفه استمتع")
	...
	client.on('message', async msg => { // Leaked by [ @FOIL#4996 ]
	var prefix = "$";
	var user = msg.author;
...
.setDescription(`**' King Bot Discord.\n$help - لرؤية الأوامر :comet: \n$inv - لدعوة البوت :wine_glass: \n معلومات عن البوت :thinking: \n بوت ديسكورت متكامل :soccer: :microphone: :earth_americas: \nيوجد داخل البوت خاصية منع التهكير مجانا وبسهوله تامة :scream: \nصيانة دورية :stopwatch: :wrench: \n 24 ساعة :point_up:
 \nاضافات يومية :link: \n الدعم الفني للمساعدةة : https://discord.gg/AQYpbjY  :rose:  \n وشكرا لكم :lizard: **`)
...
client.on('message',   eyad =>{ // Leaked by [ @FOIL#4996 ]
...
// شف دسكورد
client.on('message', msg => { // Leaked by [ @FOIL#4996 ]
